import { IsString, IsOptional, IsUUID, IsBoolean, IsDateString, MaxLength, MinLength, IsEnum, IsArray, ArrayMinSize, IsNumber, Min } from 'class-validator';
...
export class CreatePessoaDto {
  ...
  @ApiProperty({ description: 'CPF ou CNPJ (apenas números ou com formatação)', required: false, example: '12345678901' })
  @IsOptional()
  @IsString({ message: 'Documento deve ser um texto' })
  @IsCpfCnpj({ message: 'CPF ou CNPJ inválido' })
  documento?: string;

  @ApiProperty({ description: 'Inscrição Estadual ou RG', required: false, example: '123456789' })
  @IsOptional()
  ...
  @ApiProperty({ description: 'Inscrição Municipal', required: false, example: '12345' })
  @IsOptional()
  ...
  @IsOptional()
  @IsEnum(TipoContribuinte, { message: 'Tipo de contribuinte inválido. Use: 1 (Contribuinte ICMS), 2 (Isento) ou 9 (Não Contribuinte)' })
  tipoContribuinte?: TipoContribuinte;

  @ApiProperty({ description: 'Indica se a pessoa é consumidor final', default: true })
  @IsOptional()
  @IsBoolean({ message: 'Consumidor final deve ser verdadeiro ou falso' })
  consumidorFinal?: boolean;

  @ApiProperty({ description: 'Data de aniversário (formato ISO: YYYY-MM-DD)', required: false, example: '1990-01-15' })
  @IsOptional()
  @IsDateString({}, { message: 'Aniversário deve ser uma data válida no formato ISO (YYYY-MM-DD)' })
  aniversario?: string;

  @ApiProperty({ description: 'Limite de crédito disponível (em reais)', required: false, example: 5000.00, default: 0 })
  @IsOptional()
  ...
  @ApiProperty({
    description: 'Situação financeira da pessoa (ativo, inativo, bloqueado, suspenso)',
    enum: SituacaoFinanceira,
    default: SituacaoFinanceira.ATIVO,
    required: false
  })
  @IsOptional()
  @IsEnum(SituacaoFinanceira, { message: 'Situação financeira deve ser ativo, inativo, bloqueado ou suspenso' })
